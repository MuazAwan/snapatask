...
# Fix hermes shebang to use venv python
docker exec paperclip python3 -c "
content = open('/usr/local/bin/hermes').read()
if '#!/usr/bin/python3' in content:
    content = content.replace('#!/usr/bin/python3', '#!/usr/local/lib/hermes-agent/venv/bin/python3.11')
    open('/usr/local/bin/hermes', 'w').write(content)
    print('Shebang fixed')
else:
    print('Shebang already correct')
" 2>/dev/null
